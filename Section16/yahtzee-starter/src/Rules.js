...
class TotalOneNumber extends Rule {
  description = () => {
    return `${this.val} ${this.val === 1 ? 'point' : 'points'} per ${this.val}`;
  }
  evalRoll = dice => {
...
class SumDistro extends Rule {
  description = () => {
    let str = `Sum of all dice`;
    if(this.count === 3) str += ' if 3 are the same';
    if(this.count === 4) str += ' if 4 are the same';
    return str;
  }
  evalRoll = dice => {
...
class FullHouse extends Rule {
  description = () => {
    return `${this.score} points for a full house`;
  }
  evalRoll = dice => {
...
class SmallStraight extends Rule{
  description = () => {
    return `${this.score} for a small straight`;
  }
  evalRoll = dice => {
    let d = new Set(dice);
...
class LargeStraight extends Rule {
  description = () => {
    return `${this.score} for a large straight`;
  }
  evalRoll = dice => {
